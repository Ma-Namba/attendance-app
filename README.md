...
## 要件シート外の追加
### ❗コーチ判断により、提供blade(user-detail.blade.php)と提供CSS(user-detail.css)の以下内容の調整
- バリデーションメッセージをテスト要件に合わせる
- バリデーションエラーの際に、入力データが引き継がれてリダイレクトされるように修正
- ユーザー名の表示幅が80pxしかなかったので、実運用に近い日本名が全て表示されるように修正
